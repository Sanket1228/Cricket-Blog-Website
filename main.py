...
mail = Mail(app)
if(localserver):
    app.config['SQLALCHEMY_DATABASE_URI'] = params['loc_uri']
else:
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_username']):
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=title, tagline=tline, slug=slug,
                             content=content, img_url=img_file.filename, date=date)
                db.session.add(post)
                db.session.commit()
            else:
                ...
                post.img_url = img_file.filename    
                db.session.commit()
...
def contact():
    if(request.method == 'POST'):
        ...
        entry = Contacts(name=name, phone_number=phone,
